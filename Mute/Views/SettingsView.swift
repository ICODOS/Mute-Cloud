    ...
    var body: some View {
        ScrollView {
            VStack(alignment: .leading, spacing: 20) {
                ...
                // Startup Section
                VStack(alignment: .leading, spacing: 10) {
                    SettingsSectionHeader(title: "Startup", icon: "power")
...
struct LaunchAtLoginToggle: View {
    ...
    var body: some View {
        SettingsToggleRow(
            ...
            description: "Start Mute automatically so hotkeys are always available",
            isOn: $launchAtLogin
        )
        .onChange(of: launchAtLogin) { newValue in
            do {
                ...
                Logger.shared.log("Launch at login failed: \(error)", level: .error)
            ...
            if (hotkeyConfig.isModifierOnly || hotkeyConfig.isTwoKeyCombo) && hotkeyConfig.isEnabled && !hasAccessibility {
                HStack(spacing: 6) {
                    Image(systemName: "exclamationmark.triangle.fill")
                        .foregroundColor(.orange)
                        .font(.system(size: 12))
                    Text("Accessibility permission required for this shortcut type")
                        .font(.system(size: 11))
                        .foregroundColor(.orange)
                    Spacer()
                    Button("Grant Access") {
